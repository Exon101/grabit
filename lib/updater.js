...
/**
 * Wire chrome.alarms.onAlarm to trigger checkForUpdates + keepalive.
 * Call once at service-worker boot.
 */
export function wireAlarmHandler() {
  chrome.alarms.onAlarm.addListener(async (alarm) => {
    // Keepalive alarm — just touch storage to reset SW activity timer
 ...
 * This breaks: alarms, notifications, long-running downloads, content-
 * script IPC. To keep the SW alive, we create an alarm that fires every
 * 30 seconds (the minimum allowed for unpacked extensions). The handler
 * touches chrome.storage.local to reset the activity timer.
 *
 * Note: Chrome may still terminate the SW in extreme cases (memory
 * pressure, browser restart). This is a best-effort keepalive.
 * ------------------------------------------------------------------ */

const KEEPALIVE_ALARM_NAME = 'grabit-keepalive';
const KEEPALIVE_PERIOD_MIN = 0.5; // 30 seconds (minimum for unpacked)

export async function startKeepalive() {
  // Check if already exists
  const existing = await chrome.alarms.get(KEEPALIVE_ALARM_NAME);
  if (existing) {
    logger.info('Keepalive alarm already running');
    return;
  }
  ...
  logger.info(`Keepalive alarm started (every ${KEEPALIVE_PERIOD_MIN * 60}s)`);
}

export async function stopKeepalive() {
  await chrome.alarms.clear(KEEPALIVE_ALARM_NAME);
  logger.info('Keepalive alarm stopped');
}
...
/**
 * The keepalive handler — just touches storage to reset the SW's
 * activity timer. Must be fast (<5ms) to avoid being flagged as
 * expensive work.
 */
async function keepaliveTick() {
  try {
    await chrome.storage.local.get('_keepalive');
    // Optionally set a timestamp so we can verify it's running
