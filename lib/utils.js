...
export const DEFAULT_SETTINGS = Object.freeze({
  ...
  // Error reporting
  errorReporting: true,         // local capture always on; this controls remote
  sentryDsn: '',                // empty = local-only; set to send to Sentry

  // Service worker
  keepServiceWorkerAlive: true, // keep SW alive via 30s keepalive alarm
});
...
/**
 * Mapping of site key → host-permission URL pattern(s) we need.
 * Used to request optional_host_permissions on first use of each site.
 */
export const SITE_HOST_PATTERNS = Object.freeze({
...
/**
 * Check whether the extension currently has host permission for a given URL.
 * Works in MV3 with optional_host_permissions.
 */
...
/**
 * Check whether the extension has host permission for any of the patterns
 * associated with a given site key.
 */
export async function hasSitePermission(siteKey) {
  const patterns = SITE_HOST_PATTERNS[siteKey];
...
/**
 * Request host permission for a specific URL (origin-level).
 * MUST be called from a user gesture (popup click, context menu, etc.).
 * Returns true if granted.
 */
export async function requestHostPermission(url) {
  if (!url) return false;
  try {
    // Convert URL to origin pattern: https://*.example.com/*
    const u = new URL(url);
    const origin = `${u.protocol}//*.${u.hostname.replace(/^www\./, '')}/*`;
    return await chrome.permissions.request({ origins: [origin] });
  } catch (e) {
    logger.warn('requestHostPermission failed', e?.message);
    return false;
  }
}

/**
 * Request host permission for all patterns associated with a site key.
 * MUST be called from a user gesture.
 */
export async function requestSitePermission(siteKey) {
  const patterns = SITE_HOST_PATTERNS[siteKey];
  if (!patterns?.length) return false;
  try {
    return await chrome.permissions.request({ origins: patterns });
  } catch (e) {
    logger.warn('requestSitePermission failed', e?.message);
    return false;
  }
}

/**
 * Remove host permission for a site (used by the options page toggle).
 */
export async function removeSitePermission(siteKey) {
  const patterns = SITE_HOST_PATTERNS[siteKey];
  if (!patterns?.length) return false;
  try {
    return await chrome.permissions.remove({ origins: patterns });
  } catch (e) {
    logger.warn('removeSitePermission failed', e?.message);
    return false;
  }
}
